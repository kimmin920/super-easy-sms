...
import {
  emailCellClassName,
  phoneNumberCellClassName,
} from '~/lib/react-datasheet-grid/validation';
...
const createEmptyRow = (businessId: number): ClassRow => ({
...
export default function ClassDataSheet() {
  const location = useLocation();
  const params = useParams();
  const businessId = Number(params.businessId);

  const [data, setData] = useState<ClassRow[]>(
    Array.from({ length: 10 }, () => createEmptyRow(businessId))
  );
  ...
  const columns = [
    ...
    { ...keyColumn('name', textColumn), title: '이름' },
    {
      ...keyColumn('phone_number', {
        ...phoneNumberColumn,
        cellClassName: phoneNumberCellClassName,
      }),
      title: '전화번호',
    },
    {
