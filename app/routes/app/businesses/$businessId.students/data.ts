...
const fakeStudents = {
  ...
  async getAll(): Promise<ContactRecord[]> {
    return Object.keys(fakeStudents.records)
      .map((key) => fakeStudents.records[key])
      .sort(sortBy('-createdAt', 'last'));
  },

  async get(id: string): Promise<ContactRecord | null> {
    return fakeStudents.records[id] || null;
  },
  ...
  async create(values: ContactMutation): Promise<ContactRecord> {
    ...
    fakeStudents.records[id] = newContact;
    return newContact;
  },

  async set(id: string, values: ContactMutation): Promise<ContactRecord> {
    const contact = await fakeStudents.get(id);
    invariant(contact, `No contact found for ${id}`);
    const updatedContact = { ...contact, ...values };
    fakeStudents.records[id] = updatedContact;
    return updatedContact;
  },

  destroy(id: string): null {
    delete fakeStudents.records[id];
    return null;
  },
};
...
export async function getContacts(query?: string | null) {
  ...
  let contacts = await fakeStudents.getAll();
  if (query) {
...
export async function createEmptyContact() {
  const contact = await fakeStudents.create({});
  return contact;
}

export async function getContact(id: string) {
  return fakeStudents.get(id);
}
...
export async function updateContact(id: string, updates: ContactMutation) {
  ...
  await fakeStudents.set(id, { ...contact, ...updates });
  return contact;
}

export async function deleteContact(id: string) {
  fakeStudents.destroy(id);
}
